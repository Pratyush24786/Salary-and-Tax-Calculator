...
function updateSalary() {
  ...
  const monthlyWeight = 60;
  const weeklyWeight = 27;
  document.getElementById("pay-bar-month").style.width = `${monthlyWeight}%`;
  document.getElementById("pay-bar-week").style.width = `${weeklyWeight}%`;
  document.getElementById("pay-bar-hour").style.width = `${100 - monthlyWeight - weeklyWeight}%`;
}
...
function updateTax() {
  ...
  const retained = gross === 0 ? 100 : Math.max(0, Math.min(100, ((gross - total) / gross) * 100));
  document.getElementById("tax-meter-fill").style.width = `${retained}%`;
  document.getElementById("take-home-percent").textContent = `${retained.toFixed(1)}%`;
